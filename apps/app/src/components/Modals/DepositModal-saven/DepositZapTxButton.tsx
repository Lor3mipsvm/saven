...
export const DepositZapTxButton = (props: DepositZapTxButtonProps) => {
    ...
    // Gate BEFORE hook calls
    if (!userAddress || !inputTokenAddress) {
        return (
            <Button className="w-full bg-slate-700 text-slate-400 border-slate-600" disabled>
                {!userAddress ? 'Connect your wallet' : 'Loading token…'}
            </Button>
        )
    }
    ...
    // If the hook provides a target for approvals, prefer it; else fall back:
    const spender = (dataTx as any)?.approvalTarget ?? vault.address
    ...
    const {
        ...
        spender as Address,              // <-- use spender here
